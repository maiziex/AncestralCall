...
parser.add_argument('--out_vcf','-o_vcf', help="out vcf")
...
def evaluate_alu_del(vcf_file,lib_prefix,alu_event_save): 
    ...
    for line in f:
        ...
        if data[0][0] == "#":
            ...
            if ref_len > alt_len :
                ...
                if alu_flag == 1:
                    #print(data)
                    #fw.writelines("\t".join(data) + "\t" +  "alu_ins_1" + "\n")
                    count_alu += 1
                    alu_dict_flag[event_name] = "alu_del_1"
                else:
                    count_not_alu += 1
                    #fw.writelines("\t".join(data) + "\t" + "alu_del_0" + "\n")
...
def add_alu_tag_into_vcf(alu_dict_flag,del_vcf,del_vcf2):
    f = open(del_vcf,"r")
    fw = open(del_vcf2,"w")
    for line in f:
        data = line.rsplit()
        if data[0][:2] == "##":
            ...
            if event_name in alu_dict_flag:
                if alu_dict_flag[event_name] == "alu_del_1":
                    alu_flag = 1
...
if __name__ == "__main__":
    ...
    if os.path.exists(out_dir):
        ...
        os.makedirs(out_dir)
    del_vcf = in_dir + sample_name + "_DEL.vcf"
    del_vcf2 = in_dir + out_vcf
    alu_vcf = out_dir + sample_name + "_alu_del.vcf"
    alu_fasta = out_dir + sample_name + "_alu_del.fasta"
    alu_sam = out_dir + sample_name + "_alu_del.sam"

    extract_SV(del_vcf,alu_vcf,alu_fasta)
    try:
    ...
    os.system(align_alu_cmd)
    alu_event_save = evaluate_sam_file(alu_sam)
    lib_prefix = out_dir + sample_name
    alu_dict_flag = evaluate_alu_del(alu_vcf,lib_prefix,alu_event_save)
    
    add_alu_tag_into_vcf(alu_dict_flag,del_vcf,del_vcf2)
