...
parser = ArgumentParser(description="SV (DEL) calling:")
        ...
        if data[0] == "V":
            ...
            if mapq >= 20 and repeat == 1: 
                ...
                if chr_num in use_chr_list:
                    ...
                    strand_dir = data[-1]
                    contig_dict[(chr_num,ref_start,ref_end)].append([chr_num,ref_start, ref_end, ref, alt, contig_num,contig_start,contig_end,strand_dir])
...
def compare_two_haploid_SV(SV_dict_contig_1_file, SV_dict_contig_2_file,lib_prefix):
            ...
            if chr_num_1 == chr_num_2 and ((start_1 >= start_2 and end_1 <= end_2) or (start_1 >= start_2 and start_1 <= end_2) or (start_1 <= start_2 and end_1 >= start_2) or (start_1 <= start_2 and end_1 >= end_2)):
                if alt_1 == "-" and alt_2 == "-": # del
                    if key not in homo_key_dict and key_2 not in homo_key_dict_2:
                        ...
                        #del_homo_sv[(chr_num_1,start_used,end_used)] = [chr_num_1,start_used,end_used,ref_1,alt_1]  # for final overlap of 3libs
    ...
    for key, val in SV_dict_contig_1.items():
        if homo_key_dict[key] == 1:
            ...
            if alt_1 == "-":  # del
                ...
                del_hetero_sv[key] = [val[0],start_1,end_1,ref_1,alt_1,contig_num_1,contig_start_1,contig_end_1,contig_strand_dir_1,0,0,0,0]
    ...
    for key_2, val_2 in SV_dict_contig_2.items():
        if homo_key_dict_2[key_2] == 1:
            ...
            if alt_2 == "-":  # del
                ...
                del_hetero_sv[key_2] = [val_2[0],start_2,end_2,ref_2,alt_2,0,0,0,0,contig_num_2,contig_start_2,contig_end_2,contig_strand_dir_2]
    ...
    print(count_del)
    print("------")
    pickle.dump(homo_key_dict, open(lib_prefix + "_homo_key_dict.p","wb"))
    pickle.dump(homo_key_dict_2, open(lib_prefix + "_homo_key_dict_2.p","wb"))
    pickle.dump(del_homo_sv, open(lib_prefix + "_del_homo_sv.p","wb"))
    pickle.dump(del_hetero_sv, open(lib_prefix + "_del_hetero_sv.p","wb"))


def Write_vcf_for_SV(del_homo_file,del_hetero_file,vcf_output,sample_name):
...
if __name__ == "__main__":
    ...
    SV_dict_contig_1 = Extract_SV_info(lib_prefix + "_hp1.var.txt",lib_prefix + "_DEL_dict_contig_1.p",SV_len)
    SV_dict_contig_2 = Extract_SV_info(lib_prefix + "_hp2.var.txt",lib_prefix + "_DEL_dict_contig_2.p",SV_len)
    compare_two_haploid_SV(lib_prefix + "_DEL_dict_contig_1.p", lib_prefix + "_DEL_dict_contig_2.p",lib_prefix)
    Write_vcf_for_SV(lib_prefix + "_del_homo_sv.p", lib_prefix + "_del_hetero_sv.p", lib_prefix + "_DEL.vcf",sample_name)
