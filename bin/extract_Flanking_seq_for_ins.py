    ...
    for line in f:
        ...
        if data[0][0] != "#":
            ...
            if cur_chr_num == "chr" + str(chr_num):
                ...
                _end_2 = _end + 500
                sv_len = len(data[4])
                val_sv[cur_name] = [_start, _end, _start_2, _end_2,sv_len]
    pickle.dump(val_sv, open(output_file,"wb"))
    f.close()
...
def write_validated_SV_fasta(input_file,ref_chr_dict,fw,chr_num):
    ...
    for key, val in val_sv.items():
        ...
        fw.writelines(">1_chr" + str(chr_num) + "_" + key + "_" + str(sv_size) + "\n")  
        fw.writelines(seq_1_flankingseq + "\n")
        fw.writelines(">2_chr" + str(chr_num) + "_" + key + "_" + str(sv_size) + "\n")  
...
if __name__ == "__main__":
    ...
    for ii in range(1,23):
        print("chr" + str(ii))
        ref_chr_file = out_dir + "ref_seq_chr" + str(ii) + ".p"
        ref_chr_dict = pickle.load(open(ref_chr_file,"rb"))
        flanking_chr_file = out_dir + sample_name + "_ins_for_flankingseq_chr" + str(ii) + ".p"
        Extract_validated_SV(ins_vcf,flanking_chr_file,ii)
        fw = write_validated_SV_fasta(flanking_chr_file,ref_chr_dict,fw,ii)


    print("chrX")
    ref_chr_file = out_dir + "ref_seq_chr23.p"
    ref_chr_dict = pickle.load(open(ref_chr_file,"rb"))
    flanking_chr_file = out_dir + sample_name + "_ins_for_flankingseq_chr23.p"
    Extract_validated_SV(ins_vcf,flanking_chr_file,"X")
