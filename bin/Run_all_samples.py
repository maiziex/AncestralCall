...
parser.add_argument('--species_name_list','-ls', help='The species\'s name you can define. Each name is seperately by comma (",")', type=str,required=True)
parser.add_argument('--species_ref_list','-lr', help='The reference fasta files for each species which you defined in the "--species_name_list", respectivley. Each reference file is seperately by comma (",") ', type=str,required=True)
args = parser.parse_args()
if len(sys.argv) == 1:
    Popen("python Run_all_samples.py -h",shell=True).wait()
else:
