...
def Evaluate_derived_sv(bam_file,lib_prefix):
    ...
    for event_name, val in flanking_seq_dict.items():
        if len(val) == 2:
            ...
            count += 1
    pickle.dump(derived_del, open(lib_prefix + "_derived_del_by_flanking_dict.p","wb"))
    pickle.dump(derived_ins, open(lib_prefix+ "_derived_ins_by_flanking_dict.p","wb"))
...
def add_dt_tag_into_vcf(dt_dict_flag,del_vcf,del_vcf2,monkey_name):
    f = open(del_vcf,"r")
    fw = open(del_vcf2,"w")
    for line in f:
        data = line.rsplit()
        if data[0][:2] == "##":
            ...
            fw.writelines("##FORMAT=<ID=del_on_tar_" + monkey_name +  ",Number=1,Type=String,Description=\"del_on_tar flag: 1 or 0\">\n")
            fw.writelines(line)
        else:
            #data[8] = "GT:Contig:Alu:TandemRepeats:del_on_tar"
            data[8] = data[8] + ":del_on_tar_" + monkey_name
            event_name = data[2]
            if event_name in dt_dict_flag:
                dt_flag = 1
            else:
                dt_flag = 0
            data[9] = data[9] + ":" + str(dt_flag)
            fw.writelines("\t".join(data) + "\n")
    f.close()
    fw.close()


def add_ir_tag_into_vcf(ir_dict_flag,del_vcf,del_vcf2,monkey_name):
    f = open(del_vcf,"r")
    fw = open(del_vcf2,"w")
    for line in f:
        data = line.rsplit()
        if data[0][:2] == "##":
            ...
            fw.writelines("##FORMAT=<ID=ins_on_ref_" + monkey_name + ",Number=1,Type=String,Description=\"ins_on_ref flag: 1 or 0\">\n")
            fw.writelines(line)
        else:
            #data[8] = "GT:Contig:Alu:TandemRepeats:del_on_tar:ins_on_ref"
            data[8] = data[8] + ":ins_on_ref_" + monkey_name
            event_name = data[2]
            if event_name in ir_dict_flag:
                ir_flag = 1
            else:
                ir_flag = 0
            data[9] = data[9] + ":" + str(ir_flag)
            fw.writelines("\t".join(data) + "\n")
    f.close()
    fw.close()
...
if __name__ == "__main__":
    ...
    del_vcf = out_dir + in_vcf
    del_vcf2 = out_dir + sample_name + "_tmp_del.vcf"
    del_vcf3 = out_dir + out_vcf
    lib_prefix = out_dir + sample_name + "_" + monkey_name
    Evaluate_derived_sv(flanking_bam,lib_prefix)
    dt_dict_flag = pickle.load(open(lib_prefix + "_derived_del_by_flanking_dict.p","rb"))
    ir_dict_flag = pickle.load(open(lib_prefix + "_derived_ins_by_flanking_dict.p","rb"))
    add_dt_tag_into_vcf(dt_dict_flag,del_vcf,del_vcf2,monkey_name)
    add_ir_tag_into_vcf(ir_dict_flag,del_vcf2,del_vcf3,monkey_name)
